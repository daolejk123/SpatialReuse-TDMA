...
| `rl_agent.py` | `RLFeatureExtractor`（4M+10+N 维）、`LSTMActorCritic`（LSTM-1 共享 + Actor LSTM-2a + Critic LSTM-2c）、`TDMAAgent`（多节点状态管理） |
| `ppo_trainer.py` | PPO 训练循环：`RolloutBuffer`、`ppo_update()`、`train()` 主函数，支持 `--load_ckpt` 断点续训 |
| `transformer_model.py` | 离线 Transformer 回归模型；`_parse_bown`/`_parse_t2hop` 被 RL 模块复用 |

状态向量维度：`4M + 10 + N`（M = numDataSlots，N = numNodes，默认 M=10, N=9 → 维度 59）
- 节点 ID one-hot 编码（N 维）：使各节点能学习差异化策略，避免多节点协作失败
