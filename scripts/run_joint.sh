...
#   --mac_mode MODE      dynamic_tdma|heuristic_only|plain_tdma|greedy_stdma|greedy_stdma_2hop|traffic_adaptive_tdma|zmac_like|zmac_inspired|trama_like|trama_inspired|bandit_index_proxy（默认 dynamic_tdma）
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --action_mask) ACTION_MASK="$2"; shift 2 ;;
        --action_init_prob) ACTION_INIT_PROB="$2"; shift 2 ;;
        ...
        --constrained_debt) CONSTRAINED_DEBT=true; shift ;;
...
case "$MAC_MODE" in
    dynamic_tdma|heuristic_only|plain_tdma|greedy_stdma|greedy_stdma_2hop|traffic_adaptive_tdma|zmac_like|zmac_inspired|trama_like|trama_inspired|bandit_index_proxy) ;;
    *) error "mac_mode 只能是 dynamic_tdma、heuristic_only、plain_tdma、greedy_stdma、greedy_stdma_2hop、traffic_adaptive_tdma、zmac_like、zmac_inspired、trama_like、trama_inspired 或 bandit_index_proxy，当前: $MAC_MODE"; exit 1 ;;
esac
...
[ -n "$ACTION_MASK" ] && info "action_mask  = $ACTION_MASK"
[ -n "$ACTION_INIT_PROB" ] && info "action_init_prob = $ACTION_INIT_PROB"
[ -n "$SERVICE_DEBT_THRESHOLD" ] && info "service_debt_threshold = $SERVICE_DEBT_THRESHOLD"
[ -n "$SERVICE_DEBT_ACTION_BOOST" ] && info "service_debt_action_boost = $SERVICE_DEBT_ACTION_BOOST"
[ -n "$SERVICE_DEBT_REWARD_COEF" ] && info "service_debt_reward_coef = $SERVICE_DEBT_REWARD_COEF"
[ -n "$SERVICE_DEBT_MAX_FRAMES" ] && info "service_debt_max_frames = $SERVICE_DEBT_MAX_FRAMES"
[ -n "$SERVICE_DEBT_REQUEST_BUDGET" ] && info "service_debt_request_budget = $SERVICE_DEBT_REQUEST_BUDGET"
[ -n "$SERVICE_DEBT_BUDGET_BOOST" ] && info "service_debt_budget_boost = $SERVICE_DEBT_BUDGET_BOOST"
...
[ "$CONSTRAINED_DEBT" = true ] && info "constrained_debt = true"
[ -n "$CONSTRAINT_SERVICE_THRESHOLD" ] && info "constraint_service_threshold = $CONSTRAINT_SERVICE_THRESHOLD"
[ -n "$CONSTRAINT_SERVICE_COST_RAMP_FRAMES" ] && info "constraint_service_cost_ramp_frames = $CONSTRAINT_SERVICE_COST_RAMP_FRAMES"
[ -n "$CONSTRAINT_STARVATION_THRESHOLD" ] && info "constraint_starvation_threshold = $CONSTRAINT_STARVATION_THRESHOLD"
[ -n "$CONSTRAINT_STARVATION_MAX_FRAMES" ] && info "constraint_starvation_max_frames = $CONSTRAINT_STARVATION_MAX_FRAMES"
[ -n "$CONSTRAINT_DEBT_MAX" ] && info "constraint_debt_max = $CONSTRAINT_DEBT_MAX"
[ -n "$CONSTRAINT_DEBT_REPAY" ] && info "constraint_debt_repay = $CONSTRAINT_DEBT_REPAY"
[ -n "$CONSTRAINT_SERVICE_COST_LIMIT" ] && info "constraint_service_cost_limit = $CONSTRAINT_SERVICE_COST_LIMIT"
[ -n "$CONSTRAINT_STARVATION_COST_LIMIT" ] && info "constraint_starvation_cost_limit = $CONSTRAINT_STARVATION_COST_LIMIT"
[ -n "$CONSTRAINT_COST_AGGREGATION" ] && info "constraint_cost_aggregation = $CONSTRAINT_COST_AGGREGATION"
[ -n "$CONSTRAINT_TAIL_FRACTION" ] && info "constraint_tail_fraction = $CONSTRAINT_TAIL_FRACTION"
[ "$CONSTRAINT_BUDGET_ONLY" = true ] && info "constraint_budget_only = true"
[ -n "$CONSTRAINT_BUDGET_ALLOCATION" ] && info "constraint_budget_allocation = $CONSTRAINT_BUDGET_ALLOCATION"
[ -n "$CONSTRAINT_BUDGET_MIN_SHARE" ] && info "constraint_budget_min_share = $CONSTRAINT_BUDGET_MIN_SHARE"
[ -n "$CONSTRAINT_REPAY_ACTION_SLOTS" ] && info "constraint_repay_action_slots = $CONSTRAINT_REPAY_ACTION_SLOTS"
[ -n "$CONSTRAINT_REPAY_ACTION_THRESHOLD" ] && info "constraint_repay_action_threshold = $CONSTRAINT_REPAY_ACTION_THRESHOLD"
[ -n "$CONSTRAINT_REPAY_ACTION_MAX_NODES" ] && info "constraint_repay_action_max_nodes = $CONSTRAINT_REPAY_ACTION_MAX_NODES"
[ -n "$CONSTRAINT_REPAY_REWARD_COEF" ] && info "constraint_repay_reward_coef = $CONSTRAINT_REPAY_REWARD_COEF"
[ -n "$CONSTRAINT_REPAY_TARGET_PROB" ] && info "constraint_repay_target_prob = $CONSTRAINT_REPAY_TARGET_PROB"
[ -n "$CONSTRAINT_REPAY_TARGET_COEF" ] && info "constraint_repay_target_coef = $CONSTRAINT_REPAY_TARGET_COEF"
[ -n "$CONSTRAINT_LAMBDA_LR" ] && info "constraint_lambda_lr = $CONSTRAINT_LAMBDA_LR"
[ -n "$CONSTRAINT_DUAL_MAX" ] && info "constraint_dual_max = $CONSTRAINT_DUAL_MAX"
[ -n "$JOINT_VALUE_TAIL_COEF" ] && info "joint_value_tail_coef = $JOINT_VALUE_TAIL_COEF"
[ -n "$JOINT_VALUE_QUEUE_COEF" ] && info "joint_value_queue_coef = $JOINT_VALUE_QUEUE_COEF"
[ -n "$TAIL_GATE_COEF" ] && info "tail_gate_coef = $TAIL_GATE_COEF"
[ -n "$MULTIHEAD_ACTOR_TAIL_COEF" ] && info "multihead_actor_tail_coef = $MULTIHEAD_ACTOR_TAIL_COEF"
[ -n "$MULTIHEAD_ACTOR_QUEUE_COEF" ] && info "multihead_actor_queue_coef = $MULTIHEAD_ACTOR_QUEUE_COEF"
[ -n "$TRAINING_MODE" ] && info "training_mode = $TRAINING_MODE"
...
[ -n "$ACTION_MASK" ] && PPO_CMD+=(--action_mask "$ACTION_MASK")
[ -n "$ACTION_INIT_PROB" ] && PPO_CMD+=(--action_init_prob "$ACTION_INIT_PROB")
[ -n "$SERVICE_DEBT_THRESHOLD" ] && PPO_CMD+=(--service_debt_threshold "$SERVICE_DEBT_THRESHOLD")
[ -n "$SERVICE_DEBT_ACTION_BOOST" ] && PPO_CMD+=(--service_debt_action_boost "$SERVICE_DEBT_ACTION_BOOST")
[ -n "$SERVICE_DEBT_REWARD_COEF" ] && PPO_CMD+=(--service_debt_reward_coef "$SERVICE_DEBT_REWARD_COEF")
[ -n "$SERVICE_DEBT_MAX_FRAMES" ] && PPO_CMD+=(--service_debt_max_frames "$SERVICE_DEBT_MAX_FRAMES")
[ -n "$SERVICE_DEBT_REQUEST_BUDGET" ] && PPO_CMD+=(--service_debt_request_budget "$SERVICE_DEBT_REQUEST_BUDGET")
[ -n "$SERVICE_DEBT_BUDGET_BOOST" ] && PPO_CMD+=(--service_debt_budget_boost "$SERVICE_DEBT_BUDGET_BOOST")
[ "$SERVICE_DEBT_DENSITY_ADAPTIVE" = true ] && PPO_CMD+=(--service_debt_density_adaptive)
[ "$SERVICE_DEBT_DYNAMIC_BUDGET" = true ] && PPO_CMD+=(--service_debt_dynamic_budget)
[ -n "$SERVICE_DEBT_SPARSE_EDGE_DENSITY" ] && PPO_CMD+=(--service_debt_sparse_edge_density "$SERVICE_DEBT_SPARSE_EDGE_DENSITY")
[ -n "$SERVICE_DEBT_DENSE_EDGE_DENSITY" ] && PPO_CMD+=(--service_debt_dense_edge_density "$SERVICE_DEBT_DENSE_EDGE_DENSITY")
[ -n "$SERVICE_DEBT_SPARSE_SAFE_RATIO" ] && PPO_CMD+=(--service_debt_sparse_safe_ratio "$SERVICE_DEBT_SPARSE_SAFE_RATIO")
[ -n "$SERVICE_DEBT_DENSE_SAFE_RATIO" ] && PPO_CMD+=(--service_debt_dense_safe_ratio "$SERVICE_DEBT_DENSE_SAFE_RATIO")
[ -n "$SERVICE_DEBT_MASK_ENABLE_FACTOR" ] && PPO_CMD+=(--service_debt_mask_enable_factor "$SERVICE_DEBT_MASK_ENABLE_FACTOR")
[ -n "$SERVICE_DEBT_BUDGET_MIN_SCALE" ] && PPO_CMD+=(--service_debt_budget_min_scale "$SERVICE_DEBT_BUDGET_MIN_SCALE")
[ -n "$SERVICE_DEBT_BUDGET_MAX_SCALE" ] && PPO_CMD+=(--service_debt_budget_max_scale "$SERVICE_DEBT_BUDGET_MAX_SCALE")
[ -n "$SERVICE_DEBT_SUCCESS_TARGET" ] && PPO_CMD+=(--service_debt_success_target "$SERVICE_DEBT_SUCCESS_TARGET")
...
[ "$SERVICE_DEBT_WT_CONTEXT_ADAPTIVE" = true ] && PPO_CMD+=(--service_debt_wt_context_adaptive)
[ -n "$SERVICE_DEBT_WT_CONTEXT_MIN_SCALE" ] && PPO_CMD+=(--service_debt_wt_context_min_scale "$SERVICE_DEBT_WT_CONTEXT_MIN_SCALE")
[ -n "$SERVICE_DEBT_WT_NODE_DEFICIT_TARGET" ] && PPO_CMD+=(--service_debt_wt_node_deficit_target "$SERVICE_DEBT_WT_NODE_DEFICIT_TARGET")
[ -n "$SERVICE_DEBT_WT_NODE_CHANGE_TARGET" ] && PPO_CMD+=(--service_debt_wt_node_change_target "$SERVICE_DEBT_WT_NODE_CHANGE_TARGET")
[ -n "$SERVICE_DEBT_WT_EDGE_CHANGE_TARGET" ] && PPO_CMD+=(--service_debt_wt_edge_change_target "$SERVICE_DEBT_WT_EDGE_CHANGE_TARGET")
[ "$CONSTRAINED_DEBT" = true ] && PPO_CMD+=(--constrained_debt)
[ -n "$CONSTRAINT_SERVICE_THRESHOLD" ] && PPO_CMD+=(--constraint_service_threshold "$CONSTRAINT_SERVICE_THRESHOLD")
[ -n "$CONSTRAINT_SERVICE_COST_RAMP_FRAMES" ] && PPO_CMD+=(--constraint_service_cost_ramp_frames "$CONSTRAINT_SERVICE_COST_RAMP_FRAMES")
[ -n "$CONSTRAINT_STARVATION_THRESHOLD" ] && PPO_CMD+=(--constraint_starvation_threshold "$CONSTRAINT_STARVATION_THRESHOLD")
[ -n "$CONSTRAINT_STARVATION_MAX_FRAMES" ] && PPO_CMD+=(--constraint_starvation_max_frames "$CONSTRAINT_STARVATION_MAX_FRAMES")
[ -n "$CONSTRAINT_DEBT_MAX" ] && PPO_CMD+=(--constraint_debt_max "$CONSTRAINT_DEBT_MAX")
[ -n "$CONSTRAINT_DEBT_REPAY" ] && PPO_CMD+=(--constraint_debt_repay "$CONSTRAINT_DEBT_REPAY")
[ -n "$CONSTRAINT_SERVICE_COST_LIMIT" ] && PPO_CMD+=(--constraint_service_cost_limit "$CONSTRAINT_SERVICE_COST_LIMIT")
[ -n "$CONSTRAINT_STARVATION_COST_LIMIT" ] && PPO_CMD+=(--constraint_starvation_cost_limit "$CONSTRAINT_STARVATION_COST_LIMIT")
[ -n "$CONSTRAINT_COST_AGGREGATION" ] && PPO_CMD+=(--constraint_cost_aggregation "$CONSTRAINT_COST_AGGREGATION")
[ -n "$CONSTRAINT_TAIL_FRACTION" ] && PPO_CMD+=(--constraint_tail_fraction "$CONSTRAINT_TAIL_FRACTION")
[ "$CONSTRAINT_BUDGET_ONLY" = true ] && PPO_CMD+=(--constraint_budget_only)
[ -n "$CONSTRAINT_BUDGET_ALLOCATION" ] && PPO_CMD+=(--constraint_budget_allocation "$CONSTRAINT_BUDGET_ALLOCATION")
[ -n "$CONSTRAINT_BUDGET_MIN_SHARE" ] && PPO_CMD+=(--constraint_budget_min_share "$CONSTRAINT_BUDGET_MIN_SHARE")
[ -n "$CONSTRAINT_REPAY_ACTION_SLOTS" ] && PPO_CMD+=(--constraint_repay_action_slots "$CONSTRAINT_REPAY_ACTION_SLOTS")
[ -n "$CONSTRAINT_REPAY_ACTION_THRESHOLD" ] && PPO_CMD+=(--constraint_repay_action_threshold "$CONSTRAINT_REPAY_ACTION_THRESHOLD")
[ -n "$CONSTRAINT_REPAY_ACTION_MAX_NODES" ] && PPO_CMD+=(--constraint_repay_action_max_nodes "$CONSTRAINT_REPAY_ACTION_MAX_NODES")
[ -n "$CONSTRAINT_REPAY_REWARD_COEF" ] && PPO_CMD+=(--constraint_repay_reward_coef "$CONSTRAINT_REPAY_REWARD_COEF")
[ -n "$CONSTRAINT_REPAY_TARGET_PROB" ] && PPO_CMD+=(--constraint_repay_target_prob "$CONSTRAINT_REPAY_TARGET_PROB")
[ -n "$CONSTRAINT_REPAY_TARGET_COEF" ] && PPO_CMD+=(--constraint_repay_target_coef "$CONSTRAINT_REPAY_TARGET_COEF")
[ -n "$CONSTRAINT_LAMBDA_LR" ] && PPO_CMD+=(--constraint_lambda_lr "$CONSTRAINT_LAMBDA_LR")
[ -n "$CONSTRAINT_DUAL_MAX" ] && PPO_CMD+=(--constraint_dual_max "$CONSTRAINT_DUAL_MAX")
[ -n "$JOINT_VALUE_TAIL_COEF" ] && PPO_CMD+=(--joint_value_tail_coef "$JOINT_VALUE_TAIL_COEF")
[ -n "$JOINT_VALUE_QUEUE_COEF" ] && PPO_CMD+=(--joint_value_queue_coef "$JOINT_VALUE_QUEUE_COEF")
[ -n "$TAIL_GATE_COEF" ] && PPO_CMD+=(--tail_gate_coef "$TAIL_GATE_COEF")
[ -n "$MULTIHEAD_ACTOR_TAIL_COEF" ] && PPO_CMD+=(--multihead_actor_tail_coef "$MULTIHEAD_ACTOR_TAIL_COEF")
[ -n "$MULTIHEAD_ACTOR_QUEUE_COEF" ] && PPO_CMD+=(--multihead_actor_queue_coef "$MULTIHEAD_ACTOR_QUEUE_COEF")
[ -n "$TRAINING_MODE" ] && PPO_CMD+=(--training_mode "$TRAINING_MODE")
